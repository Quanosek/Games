...
export default function QuizyBoardComponent({ id }: { id: number }) {
  ...
  const GapBoard = ({ params }: { params: DataTypes }) => {
    ...
    const partsTable = params.question.split(/(\[[^\]]*\])/g).map((value, i) => ({
  ...
  return (
    <>
      ...
      >
        ...
        {data[id - 1]?.type === 'closed' && <ClosedBoard params={data[id - 1]} />}
        {data[id - 1]?.type === 'gap' && <GapBoard params={data[id - 1]} />}
        {data[id - 1]?.type === 'open' && <OpenBoard params={data[id - 1]} />}
      </div>

      <div className={styles.navigation}>
        <button disabled={id <= 0} onClick={() => router.push(`/quizy/board/${Number(id) - 1}`)}>
          <Image
            style={{ rotate: '-90deg' }}
            className='icon'
            alt='W lewo'
