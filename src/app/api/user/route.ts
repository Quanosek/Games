...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (existingUser) {
      return NextResponse.json({ message: 'Podany adres e-mail jest już zajęty' }, { status: 409 })
    }
    ...
    // return response
    return NextResponse.json({ message: 'Pomyślnie utworzono nowe konto', user }, { status: 201 })
  } catch (error) {
    // error response
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (!existingUser) {
      return NextResponse.json({ message: 'Nie znaleziono konta użytkownika' }, { status: 404 })
    }
    ...
    if (data.username) {
      ...
      if (usernameExists && usernameExists.id !== id) {
        return NextResponse.json({ message: 'Nazwa użytkownika jest już zajęta' }, { status: 406 })
      }
    }
